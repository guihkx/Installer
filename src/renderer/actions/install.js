...
async function downloadAsar() {
    try {
        ...
        const remoteAsar = releases && releases.length && releases[0].assets && releases[0].assets.find(a => a.name.toLowerCase() === "betterdiscord.asar");
        const asarUrl = remoteAsar && remoteAsar.url;
        if (!asarUrl) {
            let errMessage = "Could not get the asset url";
            if (!remoteAsar) errMessage = "Could not get asset object";
        ...
        try {
            const resp = await downloadFile(asarUrl);
            await originalFs.writeFile(asarPath, resp.body);
        }
        catch (error) {
            log(`❌ Failed to download package from ${asarUrl}`);
            log(`❌ ${error.message}`);
            return error;
        }
    }
